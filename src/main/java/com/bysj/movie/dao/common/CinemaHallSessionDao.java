package com.bysj.movie.dao.common;
/**
 * 影厅排片信息管理数据库操作层
 */
...
import com.bysj.movie.entity.common.CinemaHallSession;
@Repository
public interface CinemaHallSessionDao extends JpaRepository<CinemaHallSession, Long> {
	
	@Query(value="select count(id) from bysj_cinema_hall_session where movie_id = :movieId and start_time > DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i') GROUP BY cinema_id",nativeQuery=true)
	List<Integer> getShowTotal (@Param("movieId")Long movieId);
	
	@Query(value="select count(id) from bysj_cinema_hall_session where cinema_id = :cinemaId and start_time > DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i') GROUP BY movie_id",nativeQuery=true)
	List<Integer> getCinemaShowTotal (@Param("cinemaId")Long cinemaId);
	
	@Query(value="select * from bysj_cinema_hall_session where movie_id = :movieId and start_time > DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i') GROUP BY cinema_id",nativeQuery=true)
	List<CinemaHallSession> findDistinctCinemaList(@Param("movieId")Long movieId);
	
	@Query(value="select * from bysj_cinema_hall_session where movie_id = :movieId and start_time > DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i') GROUP BY show_date,cinema_id",nativeQuery=true)
	...
	@Query(value="select * from bysj_cinema_hall_session where cinema_id = :cinemaId and start_time > DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i') GROUP BY show_date",nativeQuery=true)
	List<CinemaHallSession> findDistinctShowDateByCinemaList(@Param("cinemaId")Long cinemaId);
	
	@Query(value="select * from bysj_cinema_hall_session where cinema_id = :cinemaId and show_date = :showDate GROUP BY movie_id",nativeQuery=true)
